...
<%!public String[] getParameterValues(javax.servlet.ServletRequest request, String name) {
   ...
   public Content fixDraft(Content theContent, Workspace ws) {
      ...
      try {
         // add the field to check for later if
         // the person doing the copying tries to approve later
         Workspace wsUser = Utils.getWorkspace();
         wsUser.useDistinguishedNames(true);
         if (theContent.hasComponent("PolicyCopier")) {
            ShortTextComponent username = (ShortTextComponent)theContent.getComponent("PolicyCopier");
            String userName = wsUser.getUserProfile().getUsername();
            username.setText(userName);
            theContent.setComponent("PolicyCopier", username);
         }
         else {
            ShortTextComponent username = (ShortTextComponent)theContent.createComponent("PolicyCopier", DocumentTypes.ShortTextComponent);
            String userName = wsUser.getUserProfile().getUsername();
            username.setText(userName);
            theContent.setComponent("PolicyCopier", username);
         }
   ...
   public boolean emailModelOwners(Document original, Document copied, Workspace ws) throws Exception {
      Logger s_log = Logger.getLogger("com.prudential.JSP");
      boolean isDebug = s_log.isLoggable(Level.FINEST);
      ...
      try {
         ...
         WCMUtils.sendMessage(props, emailUser, emailPassword, fromEmailAddress, recipientList, subject, emailBody, emailBodyType);
      ...
      if (item != null && item.trim().length() > 0) {
         try {
            ...
            if (docId != null) {
               ...
               // no longer call this, as it will be done when the copy is published
