               ...
               try {
                  ...
                  while (contentIterator.hasNext()) {
                     ...
                     if (theContent.hasComponent(p_lastRunComponent)) {
                        if (isDebug) {
                           s_log.log(Level.FINEST, "content had component, retrieving from content " + p_lastRunComponent);
                        }
                     ...
                     if (isDebug) {
                        s_log.log(Level.FINEST, "date from date component is " + theDate);
                     }
                     Date now = new Date();
                     ...
                     if (shouldSend) {
                        ...
                        if (isDebug) {
                           s_log.log(Level.FINEST, "will be sending email for doc " + tempContentId);
                        }
                        try {
                           ...
                           if (theContent.hasComponent(p_lastRunComponent)) {
                              if (isDebug) {
                                 s_log.log(Level.FINEST, "The content already has " + p_lastRunComponent + ", just set value");
                              }
                              theContent.setComponent(p_lastRunComponent, dc);
                           }
                           else {
                              if (isDebug) {
                                 s_log.log(Level.FINEST, "The content did not have " + p_lastRunComponent + ", add component");
                              }
                           ...
                           try {
                              ...
                              if (errors.length < 1) {
                                 ...
                                 if (isDebug) {
                                    s_log.log(Level.FINEST, "Exception during save");
                                    for(int x=0;x<errors.length;x++) {
                                       s_log.log(Level.FINEST, "Exception during save "+errors[x]);
                                    }
                                 }
                                 // try one more
                                 ws.login();
                                 try {
                                    if (ws.isLocked(theContent.getId())) {
                                       ws.unlock(theContent.getId());
                                    }
                                    ...
                                    if (errors2.length < 1) {
                                       ...
                                       if (isDebug) {
                                          s_log.log(Level.FINEST, "Exception during save");
                                          for(int x=0;x<errors2.length;x++) {
                                             s_log.log(Level.FINEST, "Exception during save "+errors2[x]);
                                          }
                                       }
                                    }
                                 }
                           ...
                           if (savedSuccessfully) {
                              if (isDebug) {
                                 s_log.log(Level.FINEST, "content was saved, send the email");
                              }
