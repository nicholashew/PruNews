...
public class ApplyManagersAsOwnersTask extends TimerTask {

   /** Logger for the class */
   private static Logger s_log = Logger.getLogger(ApplyManagersAsOwnersTask.class.getName());
   ...
   public void run() {
      ...
      if (isDebug) {
         s_log.entering("ApplyManagersAsOwnersTask", "run");
      }

      Repository repo = WCM_API.getRepository();
      try {
         VirtualPortalContext vctx = repo.generateVPContextFromContextPath(Utils.getVPName());
         ApplyManagersAsOwnersScopedAction vpA = new ApplyManagersAsOwnersScopedAction(s_uuid);
      ...
      if (isDebug) {
         s_log.exiting("ApplyManagersAsOwnersTask", "run");
      }
