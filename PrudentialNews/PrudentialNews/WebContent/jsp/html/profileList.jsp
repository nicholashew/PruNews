...
$(function(){
	...
	var idValue = $("#profiletokenfieldid").val();
...
ul.profile-list > li > input[type='checkbox'] {
    float:right;
	margin-right:10px;
}
.category-filter {
...
<% 
	...
	DocumentIdIterator docIdIter = ws.findByName(DocumentTypes.AuthoringTemplate, "AT - Newsletter Profile");
	
	DocumentId atProfileId = null;
	if (docIdIter.hasNext()) {
	   atProfileId = docIdIter.next();
	}
	
	AuthoringTemplate newsProfileTemplate = null;
	if(atProfileId != null) {
		newsProfileTemplate = (AuthoringTemplate)ws.getById(atProfileId);
	}
	...
	Query query = queryService.createQuery(Content.class);
  	...
  	query.addSelector(Selectors.authoringTemplateEquals(newsProfileTemplate));
  
  	ResultIterator results = queryService.execute(query);
	
	while(results.hasNext()) {
		Content contentItem = (Content)results.next();		
        String id = contentItem.getId().getId();
		String title = contentItem.getTitle();
		System.out.println ("Working with content " + title);
		DocumentId docIds[] = contentItem.getCombinedCategoryIds();
		StringBuilder sb = new StringBuilder();
		for (DocumentId docId : docIds) {
			String docIdStr = docId.getId(); 
	    	if (sb.length() > 0) sb.append(',');
	    	System.out.println ("Got ID string: " + docIdStr);
   			sb.append(docIdStr);   			
		}
		
%>
<li><label for="profile_<%= id %>"><%= title %></label> <input type="radio" name="profile" value="<%= sb %>" id="profile_<%= id %>" /> </li>
<%
	}	
%>
</ul>
