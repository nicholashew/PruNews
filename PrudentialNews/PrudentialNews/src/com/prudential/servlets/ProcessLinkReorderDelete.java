...
public class ProcessLinkReorderDelete extends HttpServlet {
   ...
   static Logger s_log = Logger.getLogger(ProcessLinkReorderDelete.class.getName());
   ...
   protected void handleRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      try {
         ...
         if(uuidDeleteString != null && uuidDeleteString.length()>0) {
            try {               
               HandleDelete vpDelete = new HandleDelete(uuidString);
               // check if we need to call the setters
      ...
      if (isDebug) {
         s_log.exiting("ProcessLinkReorderDelete()", "handleRequest was success = " + success);
      }
   }
