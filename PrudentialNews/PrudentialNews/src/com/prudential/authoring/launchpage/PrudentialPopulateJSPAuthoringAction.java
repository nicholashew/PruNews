...
public class PrudentialPopulateJSPAuthoringAction implements AuthoringAction {

   /** Logger for the class */
   private static Logger s_log = Logger.getLogger(PrudentialPopulateJSPAuthoringAction.class.getName());

   private static String DESC = "PrudentialPopulateJSPAuthoringAction used to copy html to jsp";

   private static String TITLE = "PrudentialPopulateJSPAuthoringAction";
   ...
   public String getDescription(Locale p_arg0) {
      ...
      if (isDebug) {
         s_log.exiting("PrudentialPopulateJSPAuthoringAction", "getDescription returning " + DESC);
      }
   ...
   public String getTitle(Locale p_arg0) {
      boolean isDebug = s_log.isLoggable(Level.FINEST);
      if (isDebug) {
         s_log.exiting("PrudentialPopulateJSPAuthoringAction", "getTitle returning " + TITLE);
      }
      return TITLE;
   }
   ...
   public ActionResult execute(FormContext fc) {
      boolean isDebug = s_log.isLoggable(Level.FINEST);
      if (isDebug) {
         s_log.entering("PrudentialPopulateJSPAuthoringAction", "execute " + fc);
      }
      ...
      try {
         ...
         if (theContent.hasComponent("JSP Text")) {
            HTMLComponent theHTML = (HTMLComponent) theContent.getComponentByReference("JSP Text");
      ...
      if (isDebug) {
         s_log.exiting("PrudentialPopulateJSPAuthoringAction", "execute " + fc);
      }
      return null;
   }
   ...
   public boolean isValidForForm(FormContext fc) {
      ...
      if (isDebug) {
         s_log.entering("PrudentialPopulateJSPAuthoringAction", "isValidForForm " + fc);
      }
      Properties p = System.getProperties();
      String key = "com.pru.AppServerEnv";
      // will be DEV/QA/STAGE/PROD
      String value = (String) p.get(key);
      if (value.equalsIgnoreCase("DEV") || value.equalsIgnoreCase("QA")) {
         Document theDoc = fc.document();
         if (theDoc instanceof Content) {
            Content theContent = (Content) theDoc;
            if (theContent.hasComponent("JSP") && theContent.hasComponent("JSP Text")) {
               isValid = true;
            }
         }
      }
      ...
      if (isDebug) {
         s_log.entering("PrudentialPopulateJSPAuthoringAction", "isValidForForm returning " + isValid);
      }
      return isValid;
   }
